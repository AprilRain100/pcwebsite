...
export default {
  ...
  methods: {
    ...
    initMap () {
      ...
      AMapUI.loadUI(['misc/PositionPicker'], PositionPicker => {
        ...
        // 拖拽完成发送自定义 drag 事件
        positionPicker.on('success', data => {
          // 过滤掉初始化地图后的第一次默认拖放
          if (!this.dragStatus) {
  ...
  async created () {
    ...
    if (window.AMap && window.AMapUI) {
      ...
      await remoteLoad(`http://webapi.amap.com/maps?v=1.3&key=c825530d011c7fcca34228bb37a19b71`)
...
<style lang="less">
  ...
  .info{ margin: 0; padding: 0; list-style: none; line-height: 30px; margin-left:20px; }
